...
export class CreateCuotaDTO {
  @IsNumber({ maxDecimalPlaces: 2 }, { message: 'cuota_amount debe ser un número válido con máximo 2 decimales' })
  @Min(0, { message: 'cuota_amount no puede ser negativo' })
  @IsNotEmpty({ message: 'cuota_amount es requerido' })
  @Transform(({ value }) => parseFloat(value))
  cuota_amount: number;

  @IsNumber({ maxDecimalPlaces: 2 }, { message: 'cfa_amount debe ser un número válido con máximo 2 decimales' })
  @Min(0, { message: 'cfa_amount no puede ser negativo' })
  @IsNotEmpty({ message: 'cfa_amount es requerido' })
  @Transform(({ value }) => parseFloat(value))
  cfa_amount: number;
}
